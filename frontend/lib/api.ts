...
export function getJob(id: string) {
  return request<JobSnapshot>(`/api/jobs/${id}`);
}

export function getCurriculum(subject: string) {
  return request<{ units: unknown[] }>(`/api/curriculum?subject=${encodeURIComponent(subject)}`);
}
...
export async function createJob(input: {
  ...
  subject: string;
  outputMode: string;
  consent: { aiOcrProcessing: boolean; personalDataRisk: boolean; storageRetention: boolean };
}) {
...
export function patchMetadata(jobId: string, metadata: ExamMetadata) {
  return request<JobSnapshot>(`/api/jobs/${jobId}/metadata`, {
    method: "PATCH",
...
export function patchAsset(jobId: string, assetId: string, patch: AssetPatch) {
  return request<JobSnapshot>(`/api/jobs/${jobId}/assets/${assetId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export function continueJob(jobId: string) {
  return request<{ ok: boolean }>(`/api/jobs/${jobId}/continue`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export function resolveIssue(jobId: string, issueId: string, resolution = "확인 완료") {
  return request<JobSnapshot>(`/api/jobs/${jobId}/issues/${issueId}/resolve`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
